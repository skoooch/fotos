...
def drawLinedrawingProperty(vecLD, property, lineWidth=1, fn="test"):
    """
    ...
    for i in range(int(vecLD["numContours"])):
        thisC = vecLD["contours"][i]
        if len(thisC.shape) == 1:
            thisC = thisC[None, :]
    ...
    # ax.invert_xaxis()
    # Set axis limits
    ax.set_xlim([0, vecLD["imsize"][0]])
    ax.set_ylim([0, vecLD["imsize"][1]])
    ax.invert_yaxis()
    # Title
    ax.set_title(property)
    plt.savefig(f"{fn}_{property}.png")
    # return fig, ax
