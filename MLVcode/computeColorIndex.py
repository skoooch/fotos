...
def computeColorIndex(vecLD, property):
    ...
    if property == "length":
        ...
        col = np.round(
            (allLengths - minProp) / (maxProp - minProp) * (numCols - 1) + 1
        ).astype(int)
        for c in range(int(vecLD["numContours"])):
            colorIdx.append(np.zeros(len(vecLD["contours"][c])) + col[c])
        ...
        maxProp = (
            np.max(allCurv) * 0.8
        )  # Here we're fudging the range a little so that high curvatures are emphasized more
        minProp = np.min(allCurv)
        max_min = maxProp - minProp
        for i in range(int(vecLD["numContours"])):
            ...
            colorIdx.append(
                np.minimum(
                    np.round(
                        (np.log10(rowCurv[i] + 1) - minProp) / (max_min + 1e-10) * (255)
                        + 1
                    ),
                    256,
                )
        ...
        for c in range(int(vecLD["numContours"])):
            colorIdx.append(
                np.round(
                    np.mod(vecLD["orientations"][c], 180) / 180 * (numCols - 1) + 1
                )
            )
